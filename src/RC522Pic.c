...
static void vInitRC522SPIPic(void)
{
	...
	spi_set_baudrate(&spi_master_instance,1000000);
...
static void MFRC522_Wr( char addr, char value )
{
	...
	spi_transceive_wait(&spi_master_instance, ( addr << 1 ) & 0x7E ,&LocalValue);
	spi_transceive_wait(&spi_master_instance,value,&LocalValue);
	port_pin_set_output_level(RC522_SS,true);
...
static char MFRC522_Rd( char addr )
{
	...
	uint16_t LocalValue;
	port_pin_set_output_level(RC522_SS,false);
	spi_transceive_wait(&spi_master_instance, (( addr << 1 ) & 0x7E) | 0x80 ,&LocalValue);
	spi_transceive_wait(&spi_master_instance,0x00,&LocalValue);
	port_pin_set_output_level(RC522_SS,true);
...
static char MFRC522_ToCard( char command, char *sendData, char sendLen, char *backData, unsigned *backLen )
{
	char _status = MI_ERR;
	char irqEn = 0x00;
	char waitIRq = 0x00;
...
static char MFRC522_Request( char reqMode, char *TagType )
{
	char _status;
...
static void MFRC522_CRC( char *dataIn, char length, char *dataOut )
{
	char i, n;
...
static char MFRC522_SelectTag( char *serNum )
{
	uint8_t i;
	char _status;
	char size;
...
//static char MFRC522_Auth( char authMode, char BlockAddr, char *Sectorkey, char *serNum )
//{
	//char _status;
	//unsigned recvBits;
	//uint8_t i;
...
//static char MFRC522_Write( char blockAddr, char *writeData )
//{
	//char _status;
	//unsigned recvBits;
	//uint8_t i;
...
//static char MFRC522_Read( char blockAddr, char *recvData )
//{
	//char _status;
	//unsigned unLen;
...
static char MFRC522_AntiColl( char *serNum )
{
	char _status;
	uint8_t i;
	char serNumCheck = 0;
...
static char MFRC522_isCard( char *TagType )
{
...
static char MFRC522_ReadCardSerial( char *str )
{
	char _status;
